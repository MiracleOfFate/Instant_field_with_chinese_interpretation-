#include "meshio.h"
#include <unordered_map>	// C++中的哈希表，可以在任意类型与类型之间做映射
#include <fstream>
#include "normal.h"
...
void load_mesh_or_pointcloud(const std::string & filename, MatrixXu & F, MatrixXf & V, MatrixXf & N, const ProgressCallback & progress)
{
...
/* 三角形网格读取。如果.obj里面包含一个四边形，则划分为2个三角形 */
void load_obj(const std::string &filename, MatrixXu &F, MatrixXf &V, const ProgressCallback &progress)
{
	/// Vertex indices used by the OBJ format（OBJ格式使用的顶点索引——面f中）
	struct obj_vertex {
		// 顶点索引、法向索引、纹理坐标索引
		uint32_t p = (uint32_t)-1;	// 4294967295：32位无符号整数的十进制最大值
		...
		inline obj_vertex(const std::string &string) {
			...
			if (tokens.size() < 1 || tokens.size() > 3)
				throw std::runtime_error("Invalid vertex data: \"" + string + "\"");	// 无效的顶点数据（obj格式）

			p = str_to_uint32_t(tokens[0]);			// 读取顶点索引

			// #if 后面的参数为真（非0）则执行#if 后面的模块；#if 后面的参数为假，则不执行#if 后面的模块
			// 此指令多用在调试的时候，有段代码自己不想删除，怕后面用到所以用#if 0来暂时注释掉，如果想用的话就用#if 1来开启；
			#if 0
				if (tokens.size() >= 2 && !tokens[1].empty())
					uv = str_to_uint32_t(tokens[1]);// 读取顶点纹理坐标索引

				if (tokens.size() >= 3 && !tokens[2].empty())
					n = str_to_uint32_t(tokens[2]); // 读取顶点法向索引
			#endif
		}
	...
	/// Hash function for obj_vertex（obj_vertex 的哈希函数）
	...
	// 读取详细内容（顶点、面等）
	std::vector<Vector3f>   positions;		// 顶点位置组成的集合
	//std::vector<Vector2f>   texcoords;	// 顶点纹理坐标组成的集合
	//std::vector<Vector3f>   normals;		// 顶点法向组成的集合
	std::vector<uint32_t>   indices;		// 顶点索引组成的集合
	std::vector<obj_vertex> vertices;		// 顶点
	VertexMap vertexMap;					// 顶点映射
	...
	while (std::getline(is, line_str)) {
		...
		// 面读取
			...
			// 如果有第 4 个顶点
			if (!v4.empty()) {
				/* This is a quad, split into two triangles（这是一个四边形，分为两个三角形） */
				tri[3] = obj_vertex(v4);
				tri[4] = tri[0];
				tri[5] = tri[2];
				nVertices = 6;
			}
			/* Convert to an indexed vertex list（转换为索引的顶点列表） */
	...
	// 命令窗口中 顶点数、面数、所用时间 输出
	cout << "done. (V=" << V.cols() << ", F=" << F.cols() << ", took "
		<< timeString(timer.value()) << ")" << endl;

	/*cout << "V:" << V.size() << ";" << V.cols() << ";" << V.rows() << endl;
	cout << "F:" << F.size() << ";" << F.cols() << ";" << F.rows() << endl;*/
}


void load_ply(const std::string &filename, MatrixXu &F, MatrixXf &V, MatrixXf &N, bool pointcloud, const ProgressCallback &progress)
{
	...
	*/
	...
	struct VertexCallbackData {
		...
		VertexCallbackData(MatrixXf &V, const ProgressCallback &progress): V(V), progress(progress) { }
	};

	struct FaceCallbackData {
		MatrixXu &F;
		const ProgressCallback &progress;
		FaceCallbackData(MatrixXu &F, const ProgressCallback &progress): F(F), progress(progress) { }
	};

	struct VertexNormalCallbackData {
		MatrixXf &N;
		const ProgressCallback &progress;
		VertexNormalCallbackData(MatrixXf &_N, const ProgressCallback &progress): N(_N), progress(progress) { }
	};
...
void load_pointcloud(const std::string & filename, MatrixXf & V, MatrixXf & N, const ProgressCallback & progress)
{
	std::ifstream is(filename);
	if (is.fail())
	...
	auto fetch_line = [&]() {
		...
		do {
			...
			if (is.eof())	
				throw std::runtime_error("Parser error while processing ALN file!");// 处理ALN文件时解析器错误！
	...
	for (uint32_t i = 0; i<nFiles; ++i) {
		...
		if (N_sub.cols() == 0)
			generate_smooth_normals(F_sub, V_sub, N_sub, true);	// 生成平滑的法向
...
void write_mesh(const std::string & filename, const MatrixXu & F, const MatrixXf & V, const MatrixXf & N, const MatrixXf & Nf, const MatrixXf & UV, const MatrixXf & C, const ProgressCallback & progress)
{
	std::string extension;
	if (filename.size() > 4)
		extension = str_tolower(filename.substr(filename.size() - 4));
...
void write_obj(const std::string & filename, const MatrixXu & F, const MatrixXf & V, const MatrixXf & N, const MatrixXf & Nf, const MatrixXf & UV, const MatrixXf & C, const ProgressCallback & progress)
{
	Timer<> timer;
	...
	if (N.size() > 0 && Nf.size() > 0)
		throw std::runtime_error("Please specify either face or vertex normals but not both!");// 请指定面法线或顶点法线，但不能同时指定两者！
	...
	/* Check for irregular faces（检查是否有不规则的面） */
...
void write_ply(const std::string & filename, const MatrixXu & F, const MatrixXf & V, const MatrixXf & N, const MatrixXf & Nf, const MatrixXf & UV, const MatrixXf & C, const ProgressCallback & progress)
{
	auto message_cb = [](p_ply ply, const char *msg) { cerr << "rply: " << msg << endl; };
