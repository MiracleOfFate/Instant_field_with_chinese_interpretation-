...
{
	VectorXu neighborhoodSize(V2E.size() + 1);	// （顶点数+1）* 1 
	cout << "Generating adjacency matrix .. ";
	cout.flush();
	Timer<> timer;

	// neighborhoodSize[0]=INVALID + 各个顶点的度组成一维数组
	tbb::parallel_for(
	...
	// neighborhoodSize[0]=0 + 累加相应各个顶点的度组成一维数组
	neighborhoodSize[0] = 0;
	for (uint32_t i = 0; i<neighborhoodSize.size() - 1; ++i)
		neighborhoodSize[i + 1] += neighborhoodSize[i];


	AdjacencyMatrix adj = new Link*[V2E.size() + 1];	// （顶点数+1）* 1
	uint32_t nLinks = neighborhoodSize[neighborhoodSize.size() - 1]; // nLinks：neighborhoodSize数组中的最后一个元素——（即各个顶点的度数之和）
	Link *links = new Link[nLinks];						//	Link：各个顶点的度数之和 * 1 ——（有向图：总的有向边）
	for (uint32_t i = 0; i<neighborhoodSize.size(); ++i)// 地址分配
	...
	tbb::parallel_for(
		...
		for (uint32_t i = range.begin(); i != range.end(); ++i) {
			...
			int it = 0;
			do {
				uint32_t base = edge % 3, f = edge / 3;		// base：当前边属于第几个边（0，1，2）；f：当前边属于第几个面（0，1，...,F.size()-1）
				uint32_t opp = E2E[edge], next = dedge_next_3(opp);
				if (it == 0) {
					*ptr++ = Link(F((base + 2) % 3, f));   // 当前边 的 prev边 的 源顶点索引
				}
					
				if (opp == INVALID || next != stop) {
					*ptr++ = Link(F((base + 1) % 3, f));   // 当前边 的 next边 的 源顶点索引
					if (opp == INVALID)
						break;
				}
				edge = next;
	...
	/*for (size_t i = 0; i < V2E.size() + 1; i++)	// 输出结果只取第一列
	{
	...
	/*for (size_t i = 0; i < V2E.size() + 1; i++)	// 输出地址
	{
			...
			do {
				...
				if (it == 0) {
					...
					Float cot_weight = 0.0f,
						sin_alpha = d0.cross(d1).norm();		// 面法向的模
					if (sin_alpha > RCPOVERFLOW)
						cot_weight = d0.dot(d1) / sin_alpha;

					uint32_t opp = E2E[dedge_prev_3(edge)];		// 当前边的 prev边 的半对边
					if (opp != INVALID) {
...
AdjacencyMatrix generate_adjacency_matrix_pointcloud(MatrixXf & V, MatrixXf & N, const BVH * bvh, MeshStats & stats, uint32_t knn_points, bool deterministic, const ProgressCallback & progress)
{
	...
	/* 
		...
		使用试探法估计点云的一些有用量（由于kNN查询，这是有偏差的估计，但它很方便且相当准确）
	*/
