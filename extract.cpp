...
#include "bvh.h"
//#include "cleanup.h"
#include <tbb/concurrent_vector.h>	// 并发容器
	...
	bool snap_vertices) 
{
	...
	// 基本信息（level=0）——顶点代表方向、代表位置、法向、顶点、位置约束、邻接
	...
	{
		DisjointSets dset(mRes.size());	// 对应层级（level=0）的顶点个数 * 1
		adj_new.clear();
		adj_new.resize(mRes.size());	// 对应层级（level=0）的顶点个数 * 1
		typedef std::pair<Edge, float> WeightedEdge; // 边权重

		tbb::concurrent_vector<WeightedEdge> collapse_edge_vec;
		collapse_edge_vec.reserve((uint32_t)(mRes.size()*2.5f));// 2.5倍的对应层级的顶点个数 * 1

		/* 1、识别四边形网格边函数——根据优化后的顶点代表位置距离判断，从而得到 四边形邻接关系矩阵adj_new 和 需折叠的顶点对集collapse_edge_vec */
		auto classify_edges = [&](const tbb::blocked_range<uint32_t> &range) {// 循环顶点数V.cols()次
			for (uint32_t i = range.begin(); i<range.end(); ++i) {
				while (!dset.try_lock(i))
					;

				// 遍历某个顶点的所有邻接顶点（依据邻接矩阵adj）
				for (Link *link = adj[i]; link != adj[i + 1]; ++link) {
					uint32_t j = link->id;	// 某个顶点的一个邻接顶点索引

					// 只访问每条无向边，且为（小的顶点索引i，大的顶点索引j）
					if (j < i)
						continue;

					// 相邻两个顶点的代表方向（优化后的）进行配对
					std::pair<Vector3f, Vector3f> Q_rot = compat_orientation(
						Q.col(i), N.col(i), Q.col(j), N.col(j));

					Float error = 0;
					// 相邻两个顶点的代表位置（优化后的）进行配对
					...
					Vector2i absDiff = (shift.first - shift.second).cwiseAbs();	// 相邻两个顶点的代表位置进行配对时，各自移动的整数格向量之差的绝对值

					// （1）忽略两点距离大于1个整数平移单位
					if (absDiff.maxCoeff() > 1 || (absDiff == Vector2i(1, 1) && posy == 4))
						continue; /* Ignore longer-distance links and diagonal lines for quads（忽略四边形的长距离邻接和对角线） */
					bool collapse = absDiff.sum() == 0;	// 是否需要折叠——配对后，各自移动的整数格向量相同
					// （2）两点实际上指代同一个位置代表点，即两点之间的边应被折叠
					if (collapse) {
						collapse_edge_vec.push_back(std::make_pair(std::make_pair(i, j), error));// 相邻两个顶点索引 + 顶点代表位置距离差的平方（即作为边权重）
					}
					// （3）两点是输出四边形网格中相邻的两点——四边形邻接关系矩阵adj_new
					else {
						while (!dset.try_lock(j))
							;
						adj_new[i].push_back(j);	// 顶点索引为 i 的一个邻接顶点索引j
						adj_new[j].push_back(i);	// 顶点索引为 j 的一个邻接顶点索引i
						dset.unlock(j);
					}
				}
		...
		std::vector<uint16_t> nCollapses(mRes.size(), 0);// 对应层级（level=0）的顶点个数 * 1
		/* 2、折叠四边形边函数 */
		auto collapse_edges = [&](const tbb::blocked_range<uint32_t> &range) {// 循环需折叠数collapse_edge_vec.size()次
			...
			for (uint32_t i = range.begin(); i != range.end(); ++i) {
				...
				// Lock both sets and determine the current representative ID（锁定两组并确定当前代表ID） 
				bool ignore_edge = false;
				do {
					// 交换相邻两个顶点索引值，使其为（小的顶点索引i，大的顶点索引j）
					...
					if (!dset.try_lock(edge.second)) {
						...
						// （1）如果相邻两个顶点索引值一样（即本来就是同一个顶点，不需要进行折叠）
				...
				for (auto neighbor : adj_new[edge.first]) {
					// （2）对于需折叠的两个顶点：如果索引小的顶点的四边形邻接关系矩阵adj_new中 包含 索引大的顶点
				...
				if (contained) {
					...
					nConflicts++;	// nConflicts：四边形邻接关系矩阵adj_new中 包含了 需折叠的两个顶点的邻接关系 的个数
					continue;
				}

				// temp：需折叠的两个顶点的四边形邻接关系id值，并按升序排列
				temp.clear();
				for (auto neighbor : adj_new[edge.first])
		...
		size_t nEdges = adj[mRes.size()] - adj[0];	// 各个顶点的度数之和——即完全有向图中的所有有向边数，也是无向图中边数的2倍
		...
		// 根据边权重对collapse_edge_vec（需折叠顶点对数组）排序
		if (deterministic)
		...
		/* 3、确定四边形顶点——四边形顶点个数、邻接关系 */
		cout << "Step 3: Assigning vertices .. ";
		cout.flush();

		uint32_t nVertices = 0;	// 经过折叠操作后的顶点数
		std::map<uint32_t, uint32_t> vertex_map;
		Float avg_collapses = 0;
		// 遍历四边形邻接关系矩阵adj_new——循环顶点个数mRes.size()次
		for (uint32_t i = 0; i<adj_new.size(); ++i) {
			...
			avg_collapses += nCollapses[nVertices];	// 需折叠的无向边总数
			vertex_map[i] = nVertices++;			// 折叠图（映射）
		}
		adj_new.resize(nVertices);	// 折叠后的四边形邻接关系矩阵adj_new 大小（其值不改变）
		adj_new.shrink_to_fit();
		avg_collapses /= nVertices;	// 平均折叠数=需折叠的无向边总数/经过折叠操作后的顶点数

		// 折叠后的四边形邻接关系矩阵adj_new——循环经过折叠操作后的顶点数次
		tbb::parallel_for(
		...
		if (remove_spurious_vertices) {
			...
			for (uint32_t i = 0; i<adj_new.size(); ++i) {
				...
				/* 从四边形邻接关系矩阵adj_new中删除当前顶点——即删除与当前顶点构成的所有四边形边 */
				// 某个顶点的四边形邻接关系中：删除与当前顶点的邻接关系——即删除当前顶点的所有四边形邻接关系，以及与当前顶点存在四边形邻接关系的顶点也删除与当前顶点的四边形邻接关系
		...
		/* 4、指定四边形顶点位置——根据论文中的公式 */
		cout << "Step 4: Assigning positions to vertices .. ";
		cout.flush();

		O_new.resize(3, nVertices);	// 四边形顶点位置
		N_new.resize(3, nVertices);	// 四边形顶点法向
		O_new.setZero();
		N_new.setZero();
		...
		{
			Eigen::VectorXf cluster_weight(nVertices); // 折叠后的顶点数 * 1
	...
	if (remove_unnecessary_edges) {
		...
		do {
			...
			do {
				...
				std::vector<std::tuple<Float, uint32_t, uint32_t, uint32_t>> candidates;// tuple：不同类型可存放在一起
				for (uint32_t i_id = 0; i_id<adj_new.size(); ++i_id) {// 循环四边形顶点数次
					...
					for (uint32_t j = 0; j<adj_i.size(); ++j) {
						...
						for (uint32_t k = 0; k<adj_j.size(); ++k) {
							...
							// 如果某个四边形顶点的一个邻接点的邻接信息包括该四边形顶点（即两个四边形顶点相互包含，构成完全向边）
							if (k_id == i_id)
								continue;
				...
				for (auto t : candidates) {
					...
					if (height != std::get<0>(t))
						continue;
					if ((p_i - p_j).norm() < thresh || (p_i - p_k).norm() < thresh) {// b,c
	...
	tbb::parallel_for(
		...
		[&](const tbb::blocked_range<uint32_t> &range) {
		for (uint32_t i = range.begin(); i != range.end(); ++i) {
			Vector3f s, t, p = O_new.col(i);		// 某个四边形顶点
			coordinate_system(N_new.col(i), s, t);	// s，t：切平面的两个基轴

			// 某个四边形顶点的邻接关系 排序（顺时针？2143象限）
		...
		tbb::parallel_for(
			...
			for (uint32_t f = range.begin(); f != range.end(); ++f) {
				if (posy == 4 && F(2, f) == F(3, f)) {
					/* Irregular face（不规则面） */
					if (pure_quad) /* Should never get these when subdivision is on（细分开启后，切勿获取这些内容） */
