...
/* Force usage of discrete GPU on laptops（在笔记本电脑上强制使用离散GPU） */
...
int main(int argc, char **argv) {
	...
	uint32_t knn_points = 10, smooth_iter = 2;	// 邻居个数初始为 10；平滑迭代初始为 2
	...
	try {
		for (int i = 1; i<argc; ++i) {
			...
			if (strcmp("--fullscreen", argv[i]) == 0 || strcmp("-F", argv[i]) == 0) {
				...
				if (++i >= argc) {
					cerr << "Missing thread count!" << endl;					// 缺少线程数
					return -1;
				}
				...
				if (++i >= argc) {
					cerr << "Missing smoothing iteration count argument!" << endl;// 缺少平滑迭代计数参数
					return -1;
				}
				...
				if (++i >= argc) {
					cerr << "Missing knn point count argument!" << endl;		// 缺少knn点计数参数
					return -1;
				}
				...
				if (++i >= argc) {
					cerr << "Missing crease angle argument!" << endl;			// 缺少折角参数
					return -1;
				}
				...
				if (++i >= argc) {
					cerr << "Missing rotation symmetry type!" << endl;			// 缺少旋转对称类型
					return -1;
				}
				...
				if (++i >= argc) {
					cerr << "Missing position symmetry type!" << endl;			// 缺少位置对称类型
					return -1;
				}
				...
				if (++i >= argc) {
					cerr << "Missing scale argument!" << endl;					// 缺少比例参数
					return -1;
				}
				...
				if (++i >= argc) {
					cerr << "Missing face count argument!" << endl;				// 缺少面计数参数
					return -1;
				}
				...
				if (++i >= argc) {
					cerr << "Missing vertex count argument!" << endl;			// 缺少顶点计数参数
					return -1;
				}
				...
				if (++i >= argc) {
					cerr << "Missing batch mode output file argument!" << endl;	// 缺少批处理模式输出文件参数
					return -1;
				}
				batchOutput = argv[i];
			}
				...
				if (strncmp(argv[i], "-", 1) == 0) {
					cerr << "Invalid argument: \"" << argv[i] << "\"!" << endl;	// 无效参数
					help = true;
				}
	...
	// 设置 posy = 3、4，rosy = 2、4、6
	if ((posy != 3 && posy != 4) || (rosy != 2 && rosy != 4 && rosy != 6)) {
		cerr << "Error: Invalid symmetry type!" << endl;						// 无效对称类型					
		help = true;
	}
	...
	if (nConstraints > 1) {
		// 错误：只能同时使用--scale，--face和--vertices参数之一！
