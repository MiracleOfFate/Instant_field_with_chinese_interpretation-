...
void remove_nonmanifold(MatrixXu &F, MatrixXf &V, MatrixXf &Nf) 
{
	...
	auto kill_face_single = [&](uint32_t f) {
		...
		F.col(f).setConstant(INVALID);	// 设置该面的顶点都为INVALID，即设置该面无效（无该面）
	};

	//	清理面
	auto kill_face = [&](uint32_t f) {
		// 如果是四边形面 且 面的最后两个顶点相同
	...
	for (uint32_t f = 0; f < (uint32_t)F.cols(); ++f) {
		if (F(0, f) == INVALID)
			continue;
		if (F.rows() == 4 && F(2, f) == F(3, f)) {
			/* Special handling of irregular faces（特殊处理不规则的面） */
	...
	std::function<void(uint32_t)> dfs = [&](uint32_t i) {
		...
		for (uint32_t f : VF[i]) {
			if (f_adjacent.find(f) == f_adjacent.end()) /* if not part of adjacent face（如果不是相邻面的一部分） */
				continue;
