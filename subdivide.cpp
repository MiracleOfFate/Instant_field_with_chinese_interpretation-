...
void subdivide(MatrixXu & F, MatrixXf & V, VectorXu & V2E, VectorXu & E2E, VectorXb & boundary, VectorXb & nonmanifold, Float maxLength, bool deterministic, const ProgressCallback & progress)
{
	...
	// 遍历所有半边，将长度大于指定长度的边加入queue中——存储为<半边索引，半边长度>
	...
	auto subdiv = [&](const tbb::blocked_range<uint32_t> &range) {
		for (uint32_t i = range.begin(); i<range.end(); ++i) {
			uint32_t v0 = F(i % 3, i / 3), v1 = F((i + 1) % 3, i / 3);	// 每条半边的两个顶点
			// 如果两个顶点中的任何一个为非流形顶点——非流形边
			if (nonmanifold[v0] || nonmanifold[v1])
				continue;
			Float length = (V.col(v0) - V.col(v1)).squaredNorm();		// 模的平方——长度的平方
			// 如果当前半边的长度 大于 指定的长度
			if (length > maxLength) {
				uint32_t other = E2E[i];								// 当前半边的对偶半边
				if (other == INVALID || other > i)						// 确保没有加入重复边——即2条半边算一条边，且存储边索引更小的
	...
	/*while (!queue.empty()) {
		Edge edge;
		queue.try_pop(edge);
		cout << "queue=" << edge.first<<"，"<< edge.second<< endl;
	}*/

	uint32_t nV = V.cols(), nF = F.cols(), nSplit = 0;	// 顶点数，面数，分割数
	/*
	/   v0  \
	v1p 1 | 0 v0p
	\   v1  /
	...
	*/
	...
	while (!queue.empty()) {
		...
		//cout << "e0=" << edge.first << "e1=" << edge.second << endl;
		bool is_boundary = e1 == INVALID;
		uint32_t f0 = e0 / 3, f1 = is_boundary ? INVALID : (e1 / 3);	// 两个相邻面索引
